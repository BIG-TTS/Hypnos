...
////	
	...
	I2C_read(SampleCount);				//write the Data counter to SampleCount
	I2C_stop_com();
...
//		}	
	clearNewArray();
	for(i = 0; i<(unsigned int)SampleCount; i++){	//Burst read 3 Bytes for each sample and give it to *new
 	BIO_read(wByte);
	newArray[0+3*i] =	wByte[2];
	newArray[1+3*i] =	wByte[1];
	newArray[2+3*i] =	wByte[0];
	}
	for(i = 0 ; i< 3*(unsigned int)SampleCount; i++)
		{
