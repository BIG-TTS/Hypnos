...
void BIO_read(char *ArrayPointer)
{
...
//	//Do not forget to clear the EXTI_PR1 bit by writing a 1 to it in the ISR//
	...
	I2C_write(0x0D);
	I2C_start_com(1,BIO_slv_add,1);	//Start I2C communication
	I2C_read(System_Control);
	I2C_stop_com();
		
	//--- CLEAR INTERRUPTS-----------------------------------------		
	char INTERUPT_CONTENTS[2];
	I2C_start_com(1,BIO_slv_add,0);
	I2C_write(0x00);
	I2C_stop_com();
	I2C_start_com(1,BIO_slv_add,1);
	I2C_read(&INTERUPT_CONTENTS[1]);
	I2C_stop_com(); //READ STATUS 1 to clear all interrupts 
		
	I2C_start_com(1,BIO_slv_add,0);
	I2C_write(0x01);
	I2C_stop_com();
	I2C_restart_com(1,BIO_slv_add,1);
	I2C_read(&INTERUPT_CONTENTS[2]);
  ...
  I2C_start_com(2,BIO_slv_add,0);
	...
	I2C_write(0x0D);					
	I2C_write(0x00);
	I2C_stop_com();		//Clear SHDN  
	
	
	I2C_start_com(2,BIO_slv_add,0);	//Start I2C communication
	I2C_write(0x0D);
	I2C_restart_com(2,BIO_slv_add,1);	//Start I2C communication
	I2C_read(System_Control);
	I2C_stop_com();
	...
	I2C_start_com(1,BIO_slv_add,0);
	I2C_write(0x20);
	I2C_restart_com(4,BIO_slv_add,1);
	I2C_read(PPG_CONTENTS);
	I2C_read(PPG_CONTENTS+1);
	I2C_read(PPG_CONTENTS+2);
	I2C_stop_com();
	
//	printf("PPG Configuration 1 = %d\nPPG Configuration 2 = %d\nPPG Configuration 3 = %d\n",PPG_CONTENTS[0],PPG_CONTENTS[1],PPG_CONTENTS[2]);
//	//--- LED DRIVER SETTINGS-------------------------------------
//	I2C_start_com(2,BIO_slv_add,0);
//	I2C_write(0x2A);
//	I2C_write(0x3A);
